...
2. [x] Поправить последовательность разделов через «Открыть командный интерфейс конфигурации» и перемещение подсистем в
       нужном порядке.
3. [x] Определить состав ролей: «ПолныеПрава», «ОтделПродаж», «ОтделЗакупок»; настроить права (полные — всё; остальные —
       по логике).
...
- [x] Дерево подсистем + порядок разделов: ![alt text](./images/LR_1_01.png)
- [x] Роли и права: ![alt text](./images/LR_1_02.png)
- [x] Константы: ![alt text](./images/LR_1_03.png)
- [x] Общая форма + команды: ![alt text](./images/LR_1_04.png)
- [x] Проверка в режиме «1С:Предприятие»: ![alt text](./images/LR_1_05.png)
...
***Ответ:*** *К прикладным объектам относятся объекты предметной области: Справочники, Документы, Перечисления, Планы видов
...
***Ответ:*** *Создать объект в дереве конфигурации, настроить свойства (имя, типы, состав реквизитов/табличных частей), при
...
***Ответ:*** *Для разграничения доступа пользователей к данным и функциям системы: безопасность, соблюдение правил работы,
разделение обязанностей (кто что видит/может создавать/изменять/удалять).*

6. **Для чего нужен обработчик событий?**

***Ответ:*** *Обработчик событий нужен, чтобы выполнять код в ответ на действия пользователя или события системы (нажатие
